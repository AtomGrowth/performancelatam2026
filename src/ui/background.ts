...
export function renderBackground(root: HTMLElement): void {
  ...
  root.prepend(bg);
  // Las superficies con textura (sheet, accent) leen la misma URL desde aquí.
  root.style.setProperty('--aa-texture', `url(${contourTexture})`);
}
